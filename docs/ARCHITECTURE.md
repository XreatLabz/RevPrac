...
- Storage runtime: `application.config.StorageConfig` owns `storage.backend`, `storage.sqlite-path`, and `storage.pool-maximum-size`; `adapters.storage.jdbc` owns the HikariCP datasource, Flyway migration, and JDBC-backed player profile and player rating repositories; `src/main/resources/plugin.yml` declares the runtime libraries; `BootstrapRuntime` shuts storage down after queue, match, and player teardown.
...
- Storage bootstrap is fail-closed: bad config, invalid SQLite paths, or Flyway migration failures prevent the runtime from exposing repositories.
- JDBC-backed player profiles and ratings are the only durable Phase 6A records for now; match history, stats, seasons, PostgreSQL, and import/export remain future slices.
...
The next code should add match-history settlement, stats, seasons, PostgreSQL, and import/export on top of the existing duel, match, queue, and Phase 6A durable profile/rating storage. Keep match and queue rules testable without a live server, keep Paper adapters thin, and preserve in-memory match and queue-ticket assumptions until those later persistence slices are introduced.
