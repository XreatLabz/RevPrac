...
- JDBC-backed player profiles and ratings are the only durable Phase 6A records for now; `first_seen_at` is immutable after insert, profile touches keep `last_seen_at` monotonic under clock rollback, and match history, stats, seasons, PostgreSQL, and import/export remain future slices.
